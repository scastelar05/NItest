...
function formatRoutePopup(features) {
  ...
  if (features.length > 1) {
    jQuery('<div>').addClass('popup-title').text('Rutas').appendTo(html);
  }
...
function formatStopPopup(feature) {
  ...
  if (feature.properties.stop_code ?? false) {
    jQuery('<div>')
      .html([
        jQuery('<div>').addClass('popup-label').text('Código de parada:'),
        jQuery('<strong>').text(feature.properties.stop_code),
      ])
      .appendTo(html);
  }

  jQuery('<div>').addClass('popup-label').text('Rutas atendidas:').appendTo(html);
